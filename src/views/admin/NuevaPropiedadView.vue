...
<template>
        <v-card class="mx-auto my-10 p-5 " flat max-width="800">
                ...
                <v-form class="mt-10">
                        <v-text-field class="mb-5" label="Titulo de Propiedad" v-model="titulo.value.value"
                                :error-messages="titulo.errorMessage.value" />

                        <v-file-input class="mb-5" label="Fotografía" prepend-icon="mdi-camera" accept="image/jpeg"
                                v-model="imagen.value.value" :error-messages="imagen.errorMessage.value"
                                @change="uploadImage" />

                        <div class="my-5" v-if="image">
                                <p class="font-weight-bold">Imagne de Propiedad: </p>
                                <img class="w-50" :src="image" alt="Imagen" />
                        </div>
                                ...
                                <div style="height:600px">
                                        <LMap v-model:zoom="zoom" :center="center" :use-global-leaflet="false">
                                                ...
                                                <LTileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                                                        layer-type="base" name="OpenStreetMap"></LTileLayer>
                                        </LMap>
                                </div>
                        </div>

                        <v-btn block color="pink-accent-3" class="py-5 mt-10" @click="submit">
                                Agregar Propiedad
                        </v-btn>
